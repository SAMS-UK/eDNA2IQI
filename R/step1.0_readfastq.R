...
#5.Check raw data quality if set TRUE (default)----
  ...
  #outputs quality score graphics to folder (no return)
	if (doCheck) {step1.01_quality_check(folder)
  }
...
# Check if batch.csv already created, and skip if TRUE
	...
	#error check, only writes if a dataframe returned
      if(inherits(ASV_reads_batch, "data.frame"))
        {
	#batch_name is e.g. "ASV_reads_batch_1.csv";batch_name2 is the rda equivalent.
	#writing a csv file of ASV reads to outputData folder, per batch of denoised samples, as CSV and rda files.
