#' Predict IQI values and test applicability of taxa community
#'
...
#' @export
#' @section Example usage: myPREDs <- step3.0_predict_iqi(folder =
#'   "C:/full/file/path/to/fastqs/", S16_reads = myTAXA)
#'
step3.0_predict_iqi_multiple <- function(
  folder,
  AnnotatedASVs,
  auto_download = FALSE,
  ...
  potenRfs <- dir(
    file.path(find.package("eDNA2IQI"), "extdata/"),
    full.names = TRUE
  )
  ...
  RF <- subset(
    potenRfs,
    grepl(glob2rx("*/*_BMB_Run*.rda"), potenRfs, ignore.case = TRUE)
  )
  LOB <- readRDS(RF)
  ...
  if (!rlang::is_empty(too_low)) {
    ...
    messageColour(
      " for the following samples. IQI was not calculated.\n",
      "warning"
    )
    ...
    AnnotatedASVs <- AnnotatedASVs[
      !rownames(AnnotatedASVs) %in% names(too_low),
    ]
  }
  ...
  set.seed(123)
  rare_data <- suppressWarnings(as.data.frame(vegan::rrarefy(
    AnnotatedASVs,
    rarefaction_rate
  )))
  ...
  collated_predictions <- data.frame(matrix(nrow = nrow(rare_data), ncol = 0))

  for (i in 1:length(LOB)) {
    RF_final_reduced <- LOB[[i]][[1]]
    #only keeps columns in rare_data that are used in the random forest
    rare_data2 <- rare_data[, c(
      colnames(rare_data) %in% RF_final_reduced$xNames
    )]
  ...
  collated_predictions$SdevIQI <- SIG(apply(
    collated_predictions[1:length(LOB)],
    1,
    sd,
    na.rm = TRUE
  ))
  collated_predictions$MaxIQI <- SIG(apply(
    collated_predictions[1:length(LOB)],
    1,
    max,
    na.rm = TRUE
  ))
  collated_predictions$MinIQI <- SIG(apply(
    collated_predictions[1:length(LOB)],
    1,
    min,
    na.rm = TRUE
  ))
  collated_predictions$RangeIQI <- SIG(
    collated_predictions$MaxIQI - collated_predictions$MinIQI
  )
  ...
  collated_predictions$SampleID <- sub(
    "_R1.*",
    "",
    collated_predictions$SampleID,
    ignore.case = TRUE
  )
  ...
  final_data$Denoised_Reads <- readcounts_df$Denoised_Reads[match(
    rownames(final_data),
    rownames(readcounts_df)
  )]
  ...
  training_data_file <- list.files(
    system.file("extdata/bmb_training_data", package = "eDNA2IQI"),
    full.names = TRUE
  )[1]
