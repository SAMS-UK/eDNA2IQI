...
step3.0_predict_iqi_multiple <- function(folder, AnnotatedASVs, auto_download = FALSE) {
  ...
  #get line eq for slope correction
  eq= subset(potenRfs, grepl(glob2rx("*/*_eq.rda"), potenRfs, ignore.case = TRUE))
  load(eq)
  a2 <- signif(coef(correction_fit)[1], 3)   # intercept
  b2 <- signif(coef(correction_fit)[2], 3)   # slope
  ...
  collated_predictions$intercept <- a2
  collated_predictions$slope <- b2
  collated_predictions$slope_corrected_prediction <- collated_predictions$slope * collated_predictions$MeanIQI + collated_predictions$intercept
