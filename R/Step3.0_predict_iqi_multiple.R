...
#'
step3.0_predict_iqi_multiple <- function(folder, AnnotatedASVs) {
  ...
  if (!file.exists(file.path(find.package("eDNA2IQI"), "RF_Model", RFModel_rarefy_5000_taxalevel_family_BMB_Run1_4b.rda))) {
  ...
  name_list <- list()
  for (i in seq_along(LOB)) {
    # Check if MyBestRF_object exists
    if (!is.null(LOB[[i]]$MyBestRF_object)) {
      # Append xNames to the list
      name_list[[i]] <- LOB[[i]]$MyBestRF_object$xNames
    }
  }
  
  name <- unique(unlist(name_list))
  ...
  if (!is.null(excluded_samples)) {
    final_data <- rbind(collated_predictions,excluded_samples)
  } else {
