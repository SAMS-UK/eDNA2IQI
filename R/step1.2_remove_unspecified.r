...
step1.2_remove_unspecified <- function(folder, raw_files) {
  ...
  # Skip step if files already present
  ...
  for (x in seq_along(fwd_input)) {
    ...
    #message to R console
  if (length(names) > 0) {
    messageColour("Low raw read count (<15000 reads) for the following samples.\n", "warning")
...
cat("\n")
  ...
  sink(dataCleanfile, append = TRUE)
  cat(as.character(Sys.time()))
  cat("\nLow raw read count (<15000 reads) for following samples.\n")
